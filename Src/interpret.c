...
parse_single_command( char *buf, Analysis *analy )
{
    ...
    int sz[3], nodes[3];
    int i_args[4];
    int *p_hide_qty, *p_disable_qty;
    int *p_hide_elem_qty, *p_disable_elem_qty;

    char tokens[MAXTOKENS][TOKENLENGTH];
    char tmp_tokens[MAXTOKENS][TOKENLENGTH];
    ...
    float val;
    float pt[3], vec[3], rgb[3], pt2[3], pt3[3], pt4[3];
    ...
    /* Unless echo command, check for aliases and make substitutions. */
    if ( strncmp("echo", tokens[0], 4) )
        alias_substitute( tokens, &token_cnt );

    //mat name substitution here
    sucsub = mat_name_sub(analy,tokens,&token_cnt);
    ...
    if(sucsub)
    {
        ...
        else if ( strcmp( tokens[0], "hilite" ) == 0 ) /* IRC */
        {
            ...
            if ( token_cnt == 3 && is_numeric_token( tokens[2] ) )
            {
                ...
                if ( (ival) == analy->hilite_label && p_mo_class == analy->hilite_class )
                {
                  ...
                  || strcmp(tokens[0], "deselect") == 0)
        {
            ...
            rval = htable_search( MESH( analy ).class_table, tokens[1], FIND_ENTRY, &p_hte );
            if ( rval != OK )
            {
                string_to_upper( tokens[1], token_upper );
                rval = htable_search( MESH( analy ).class_table, token_upper, FIND_ENTRY, &p_hte );
                ...
                for ( i = 2; i < token_cnt; i++ ){
                        ...
                        for( obj = obj_min; obj <= obj_max; obj++ )
                        {
                            if( superclass != G_SURFACE )
                            {
                                if ( obj < 1 || obj > obj_max ||
                                        get_class_label_index( p_mo_class, obj ) == M_INVALID_LABEL )
                                {
                    ...
                    else
                    {
                        if( superclass != G_SURFACE )
                        {
                            ...
                            if ( j < 1 || j > obj_max ||
                                    (temp_ival=get_class_label_index( p_mo_class, j )) == M_INVALID_LABEL )
                            {
                                popup_dialog( INFO_POPUP,
                                              "Invalid ident (%d) for class %s (%s); ignored.",
                        ...
                        else
                        {
                             ...
                             * candidate already exists. */
                            for ( p_so = analy->selected_objects; p_so != NULL; NEXT( p_so ) )
                                if ( p_so->mo_class == p_mo_class && (p_so->ident == j || p_so->label == j) )
                                    break;
            ...
            else
            {
                ...
                else if ( rval == OK && token_cnt > 2 )
                {
                    ...
                    for ( i = 2; i < token_cnt; i++ )
                    {
                        ...
                        while ( p_so != NULL )
                        {
                            if ( p_so->mo_class == p_mo_class && p_so->label == j)
                            {
                                DELETE( p_so, analy->selected_objects );
                                selection_cleared = TRUE;
                                break;
                ...
                {
                    popup_dialog( USAGE_POPUP, "clrsel [<class name> [<ident>...]]" );
                    valid_command = FALSE;
                }
            }
        ...
        else if ( strcmp( tokens[0], "switch" ) == 0 || strcmp( tokens[0], "sw" ) == 0 )
        {
         ...
         * INCLUDE/EXCLUDE
         ****************************************/
        else if ( strcmp( tokens[0],  "exclude" ) == 0 || strcmp( tokens[0], "include" ) == 0 )
        {
                        ...
                        Bool_type *p_renorm )
{
    ...
    if ( reading_materials )
    {
        ...
        while( i < cnt && (is_numeric_token( tokens[i] ) || is_numeric_range_token( tokens[i] )) )
        {
            ...
            if ( is_numeric_range_token( tokens[i] ) )
            {
                ...
                mtl = mat_min ;
                for (mtl = mat_min ; mtl <= mat_max ; mtl++)
                {
    ...
    for ( ; i < cnt; i++ )
    {
        ...
        else if ( mat )
        {
            ...
            else if ( strcmp( tokens[i], "gslevel" ) == 0 || strcmp( tokens[i], "gs" ) == 0 )
            {
                i++;
                ...
                Bool_type *p_renorm )
{
    ...
    while( parsing )
    {
        ...
        {
            read = parse_embedded_surf_cmd( analy, tokens + i, cnt, p_renorm );
...
list_alias( )
{
    ...
    if ( alias_count>0 )
        popup_dialog( INFO_POPUP, alias_names );
...
alias_substitute( char tokens[MAXTOKENS][TOKENLENGTH], int *token_cnt )
{
    ...
    Bool_type found_alias=FALSE;
    Bool_type match_alias=FALSE;
    Alias_obj *alias;

    /* If the command is an "alias" or "unalias" command, don't do any alias substitutions! */
...
is_numeric_range_token( char *num_string )
{
     ...
     * max mat.
     */
    for ( i = 1; i < strlen(temp_string); i++ )
    {
