...
inline int enum_to_idx( GLenum mode )
{
...
inline int idx_to_enum( int idx )
{
  //assert( idx >= 0 && idx <= 3 );
  GLenum enums[] = { GL_MODELVIEW, GL_PROJECTION, GL_TEXTURE, GL_COLOR };
  return enums[idx];
}
...
void mat_init( )
{
  ...
  for(midx = 0; midx < 4; ++midx)
  {
    ...
    glGetFloatv( idx_to_enum( midx ), from_gl );
    array_to_mat( from_gl, &(gl_mats[midx]->tmat) );
  }
}

inline const Transf_mat * get_mode_matrix( GLenum mode )
{
  return &(gl_mats[enum_to_idx(mode)]->tmat);
}

void matrix_mode( GLenum mode )
{
  glMatrixMode( mode );
  enum_to_idx( mode );// just to check the mode is valid
  gl_mat_mode = mode;
}
...
{
  glPushMatrix();
  Matrix_stack * current_mat = gl_mats[enum_to_idx( gl_mat_mode )];
  ...
  gl_mats[enum_to_idx( gl_mat_mode )] = new_mat;
}
...
void pop_matrix( )
{
  ...
  gl_mats[enum_to_idx( gl_mat_mode )] = next_mat;
  next_mat->prev = NULL;
  free(current_mat);
}
...
{
  glLoadIdentity();
  mat_copy(&( gl_mats[enum_to_idx( gl_mat_mode )]->tmat),&ident_matrix);
}

void translate( float x, float y, float z )
{
  glTranslatef( x, y, z );
  Matrix_stack * glmv = gl_mats[enum_to_idx( gl_mat_mode )];
  float (* cm)[4][4] = &(glmv->tmat.mat);
  (*cm)[0][3] = (*cm)[0][0] * x + (*cm)[0][1] * y + (*cm)[0][2] * z + (*cm)[0][3];
  (*cm)[1][3] = (*cm)[1][0] * x + (*cm)[1][1] * y + (*cm)[1][2] * z + (*cm)[1][3];
  (*cm)[2][3] = (*cm)[2][0] * x + (*cm)[2][1] * y + (*cm)[2][2] * z + (*cm)[2][3];
  (*cm)[3][3] = 1.0;
}
...
inline void translateav( float a, float xyz[3] )
{
  translate( a*xyz[0], a*xyz[1], a*xyz[2] );
}
...
inline void scaleav( float a, float xyz[3] )
{
  scale( a*xyz[0], a*xyz[1], a*xyz[2] );
}
